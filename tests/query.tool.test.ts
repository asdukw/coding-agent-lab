...
import type { ModelClient, ModelRequest, ModelStreamEvent } from '../src/model/client'
...
test('query executes a tool call and round-trips the result back to the model', async () => {
  ...
  expect(terminal?.state.observations).toEqual([
    { tool: 'add', args: { a: 2, b: 3 }, ok: true, output: JSON.stringify({ sum: 5 }) },
  ])
  expect(terminal?.state.lastToolCall).toEqual({ name: 'add', args: { a: 2, b: 3 } })
})
...
test('query rejects arguments that fail the tool input schema before calling it', async () => {
  ...
  let terminal
  for await (const event of query({ initialState, model, tools: [trackedAddTool] })) {
    if (event.type === 'terminal') {
      terminal = event.terminal
    }
  }
